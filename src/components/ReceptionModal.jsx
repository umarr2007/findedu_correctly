...
const ReceptionModal = ({ isOpen, onClose, centerId }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      axios
        ...
        .catch(() => setMessage("Yo'nalishlarni yuklashda xatolik"));
    }
  }, [isOpen]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.majorId) {
      setMessage("Yo'nalishni tanlang");
      return;
    }
    if (!formData.date || !formData.time) {
      setMessage("Sana va vaqtni to'g'ri kiriting");
      return;
    }
    ...
    try {
      ...
      const reception = res.data?.data;
      if (reception && reception.id && reception.status) {
        setReceptionData(reception);
        setMessage("Darsga yozildingiz, sizni kutamiz!");
      } else if (res.data?.message) {
        setMessage(res.data.message);
      } else {
        setMessage("Ma'lumot qaytmayapti");
      }
    } catch (err) {
      if (err.response?.status === 422) {
        const msg = Object.values(err.response.data.errors || {})
          .flat()
          .join(", ");
        setMessage(msg || "Formani to'ldirishda xatolik");
      } else {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setMessage("Yozilish bekor qilindi");
